...
public class WebController {
    ...
    public ResponseEntity<List<Post>> listPosts() {
        List<Post> posts = postService.showUnApprovedPosts();
        if(posts.isEmpty()){
	...
	@RequestMapping(value = "/show/post/{id}", method = RequestMethod.GET)
    public ResponseEntity<List<Post>> listAllApprovedPosts(@PathVariable("id") int id) {
        
		List<Post> posts = postService.showOriginalPosts(id);
        if(posts.isEmpty()){
            return new ResponseEntity<List<Post>>(HttpStatus.NO_CONTENT);//You many decide to return HttpStatus.NOT_FOUND
        }
        return new ResponseEntity<List<Post>>(posts, HttpStatus.OK);
    ...
    public ResponseEntity<List<Forum>> listAllApprovedForumsByFriends(Principal p) {
	 ...
	 @RequestMapping(value = "/admin/post/{postId}", method = RequestMethod.PUT)
	    public ResponseEntity<Post> updatePost(@PathVariable("postId") int postId, @RequestBody Post post) {
	        System.out.println("Updating Forum " + postId);
	         
	        Post currentPost = postService.showPost(postId);
	         
	        if (currentPost==null) {
	            System.out.println("Blog with id " + postId + " not found");
